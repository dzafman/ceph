...
bool OSDMonitor::preprocess_command(MMonCommand *m)
{
  ...
  if (prefix == "osd stat") {
    ...
    string poolstr, objstr, nspacestr;
    cmd_getval(g_ceph_context, cmdmap, "pool", poolstr);
    cmd_getval(g_ceph_context, cmdmap, "object", objstr);
    cmd_getval(g_ceph_context, cmdmap, "nspace", nspacestr);
    ...
    object_locator_t oloc(pool, nspacestr);
    object_t oid(objstr);
    ...
    string fullobjname;
    if (!nspacestr.empty())
      fullobjname = nspacestr + string("/") + oid.name;
    else
