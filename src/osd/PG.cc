...
void read_log(ObjectStore *store, coll_t coll, PG::OndiskLog &ondisklog,
 pg_info_t info, PG::IndexedLog &log, hobject_t log_oid, OSDService *osd)
{
  // load bounds
  ...
  if (ondisklog.head > 0) {
    ...
    if (bl.length() < ondisklog.length()) {
	  ...
	  << ondisklog.length();
      throw PG::read_log_error(oss.str().c_str());
    }
...
{
  (void)read_info(store, bl, info, past_intervals, coll, biginfo_oid,
    snap_collections);
