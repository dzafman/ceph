...
int get_object_rados(librados::IoCtx &ioctx, bufferlist &bl)
{
  ...
  string msg("Write");
  // We can use an old export since the category will be empty string
  // which is what you get without a category anyway.
  int ret = ioctx.create(ob.hoid.hobj.oid.name, true, ob.oi.category);
  if (ret && ret != -EEXIST) {
